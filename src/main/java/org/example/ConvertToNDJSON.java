...
//        String destinationBucketName = "appflow-connectors-data";
        ...
        String destinationKeyPrefix = "Workday_CSV/combined5555";
        ...
        if (records.size() > 1) {
            ...
            // Convert JSON string to NDJSON string
            String ndjsonString = convertToNDJSON(jsonString);
            System.out.println("<<<<<<<<<ndjson string"+ndjsonString);

            // Upload NDJSON file to S3
            try (InputStream inputStream = new ByteArrayInputStream(ndjsonString.getBytes())) {
                ObjectMetadata metadata = new ObjectMetadata();
                metadata.setContentLength(ndjsonString.getBytes().length);
                ...
                System.out.println("<<<<NDJSON file uploaded to S3: " + destinationBucketName + "/" + destinationKeyPrefix);
            } catch (IOException e) {
                e.printStackTrace();
            }
        } else {
    ...
    public static String convertToNDJSON(String jsonString) throws IOException {
        ObjectMapper objectMapper = new ObjectMapper();
        @SuppressWarnings("unchecked")
        List<Object> objects = objectMapper.readValue(jsonString, List.class);
        StringBuilder ndjsonString = new StringBuilder();
        try {
            for (Object object : objects) {
                String json = objectMapper.writeValueAsString(object);
                ndjsonString.append(json).append("\n");
            }
        } catch (Exception e) {
            System.out.println(e.getMessage());
        }
        return ndjsonString.toString();
    }
}
